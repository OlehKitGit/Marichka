...
# Конфигурация
BOT_TOKEN = "7207983638:AAHV6AxHGA2ZnAv3KMJ1-GFJVIB-0nKRyUk"  # Замените на токен бота
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
   ...
   # else:
    await update.message.reply_text("✅ Узнай всю правду!")
    return
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    tarot_dir = Path('/root/paintings/venv311/TarotR')
    ...
    if card1 and card2 and card3:
        await update.message.reply_text("Предпосылка")
        await update.message.reply_photo(card1)
        await update.message.reply_text("Следствие")
        await update.message.reply_photo(card2)
        await update.message.reply_text("Шаг")
...
def main() -> None:
    ...
    TOKEN = "7207983638:AAHV6AxHGA2ZnAv3KMJ1-GFJVIB-0nKRyUk"
